...
const PreviousAppointments = () => {
    ...
    const getPreviousAppointments = async () => {
        try {
            ...
            const res = await axios.get(`http://localhost:4000/patient-api/perviousAppointment/${fullName}`);
    ...
    const handleOpenFeedback = (appointment) => {
        ...
        if (!appointment.doctorId) {
            ...
            axios.get(`http://localhost:4000/patient-api/perviousAppointment/${currentpatient.FirstName} ${currentpatient.LastName}`)
                .then(res => {
                    if (res.data.message === "Previous appointments") {
    ...
    const handleSubmitFeedback = async () => {
        try {
            ...
            const response = await axios.post('http://localhost:4000/patient-api/feedback', feedbackPayload);
            
            if (response.data.message === "Feedback submitted successfully") {
                // Update doctor rating in the backend
                if (selectedAppointment.doctorId) {
                    try {
                        await axios.post(`http://localhost:4000/doctor-api/update-rating/${selectedAppointment.doctorId}`, {
